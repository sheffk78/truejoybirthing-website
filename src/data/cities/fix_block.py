...
# Read entire cities.ts
with open("src/data/cities.ts", "r") as f:
    lines = f.readlines()

# Define the correct port-st-lucie-fl block
...
for i, line in enumerate(lines):
    if 'port-st-lucie-fl' in line and '"port-st-lucie-fl":' in line and i > 3790:
        start_idx = i
        print(f"Found start at line {i+1}: {repr(line[:80])}")
...
if start_idx is not None and end_idx is not None:
    ...
    print(f"\nReplaced block lines {start_idx+1}-{end_idx+1}")
